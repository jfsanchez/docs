...
```
acl setuser usuarioredis >contrasinal123inseguro on allchannels allkeys +get +set +del +info +scan +exists +hset +type +hlen +hscan +hdel +sadd +srem +scard +sscan +sismember +slowlog|get +config|get
```
